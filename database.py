...
from supabase_client import supabase  # certifique-se que este arquivo esteja correto
...
class AnalyseDatabase:
    ...
    def inserir_applicant_supabase(self, dados: Dict) -> List[Dict]:
        response = supabase.table("applicants").insert(dados).execute()
        if response.status_code >= 400:
            raise Exception(f"Erro na inserção: {response.json()}")
        return response.data

    def atualizar_applicant_supabase(self, dados: Dict) -> List[Dict]:
        if "id" not in dados:
            raise ValueError("O campo 'id' é obrigatório.")
        dados_sem_id = dados.copy()
        del dados_sem_id["id"]
        response = supabase.table("applicants").update(dados_sem_id).eq("id", dados["id"]).execute()
        if response.status_code >= 400:
            raise Exception(f"Erro na atualização: {response.json()}")
        return response.data
